...
(function($) {
	...
	function condenseNav(topLevelSummary) { // can be called repeatedly for updating
		...
		var cartHandleText = window._W.tl('Cart') + " (<span id='" + CART_NUM_ID + "'></span>)";
			...
			if (!cartHandle) {
				...
				if (getInEditor()) {
					...
					if (!config.chromeless) {
						...
						cartHandleToManipulate
							.attr('data-content', window._W.tl('The shopping cart is only visible on your published site.'))
							.popover({
								trigger: 'hover',
								placement: 'bottom',
		...
		var loginString = Weebly.allowMemberRegistration ? window._W.tl('Log In | Register') : window._W.tl('Log In');
		...
		if (getInEditor()) {
			if (memberLogin) {
				...
				if (!config.chromeless) {
					loginLink
						.attr('data-content', window._W.tl('Membership Log In only works on your published site.'))
						.popover({
							trigger: 'hover',
							placement: 'bottom',
							container: 'body',
	...
	function renderMoreItemEl() {
		return renderExtraItemEl(window._W.tl('more...'));
	}
